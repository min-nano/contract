...
- **締結完了は Webhook で受ける。** クラウド契約 API v2 に `POST /webhooks` があるため、
  当初想定していた締結状況のポーリングは不要（→ [04-external-services.md §1](04-external-services.md)）。
  Webhook 受信エンドポイントは Cloud Run 上に置き、**署名検証と冪等処理**（イベントIDでの重複排除）を
  Stripe と同じ方針で実装する。検証方式は API 到達後に確認する。
  なお v2 に到達できるまでは（§1.3 の403）、フォールバックとして手動確認で運用する。
...
> **⚠ この方針は再検討中。** 決定理由の一つだった「締結済PDF本体をAPIで取得できない」が、
> クラウド契約 API v2 の `GET /contracts/{id}/document` により**成立しなくなった**
> （→ [04-external-services.md §1](04-external-services.md)）。
> 自前保管という選択肢が復活したため、下記 §6 の課題1（解約・退会時のロックイン）への
> 対応方針とあわせて adr/0004 を見直す。**本節の記述は見直し完了までの暫定。**
...
| # | 課題 | 内容 |
| --- | --- | --- |
| 1 | **解約・退会時のデータ** | MF公式FAQによれば、有償契約を解約しても登録データは削除されないが**無償利用状態に移行し、閲覧・出力ができなくなる場合がある**。退会（事業者削除）後はダウンロード不可。→ [解約FAQ](https://biz.moneyforward.com/support/plan/faq/te07.html)。**保存義務は自事務所に残る**ため、解約・移行時の一括エクスポート手段を事前に確認し、運用手順に落とす。**API v2 の `GET /contracts/{id}/document` で締結済PDFを取得できるため、MFの一括エクスポート手段に依存せず自前で退避する道が開けた**（→ adr/0004 の再検討） |
