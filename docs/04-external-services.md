...
| 3 | **締結済PDF本体をダウンロードするエンドポイントがない** | 取得できるのは合意締結証明書のみ。自システムには「送信したPDF（＝締結対象の同一ファイル）＋合意締結証明書」を保管する。締結済PDF（署名情報が埋め込まれたもの）が必要な場合はMFの画面または通知メールから取得する運用とし、Phase 0 で取得手段を確認する |
...
改正法は「報酬の根拠を明らかにする見積書」の作成を求めるが、これは**努力義務**である
（→ [01-legal-requirements.md §1](01-legal-requirements.md)）。
...
| 12 | **改正法の条文における見積書の規定が努力義務か義務か** | 見積機能を強制するか警告に留めるか | 国土交通省の公布を待つ |
| 13 | 改正法の施行政省令の内容（記載事項の変更有無） | 法定必須項目 | 国土交通省の公布を待つ |
