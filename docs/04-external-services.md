...
| **マネーフォワード クラウドBox** | **保存先の候補。** 電帳法対応（電子取引データ保存・スキャナ保存）に対応済みで、手で上げる運用なら今日から使える。**API は提供されているがトライアル**（→ §3） |
...
## 3. マネーフォワード クラウドBox — **電帳法は満たす。残る論点は API だけ**

> **2026-08-17 改訂。** 公式ページに到達できたので、一次ソースで確認し直した。
> **電帳法の要件充足は前提として置いてよい**（そのように設計されたサービスである）。
> **未確定として残るのは「API が実際に使えるか」の 1 点**であり、それは
> **保存の要件の問題ではなく、手作業を減らす選択肢が増えるかどうかの問題**である。

**電帳法対応のストレージとして、契約書PDFの保存先の候補になる。**
使う場合、**電子取引データの真実性の確保が「事務処理規程の運用」から
「Boxへ上げる」に移る**（→ [07-operations.md §4.5](07-operations.md)）。

### 確認できた事実【文書 2026-08-17 取得】

| # | 内容 | 出どころ |
| --- | --- | --- |
| 1 | 電帳法対応のストレージサービスで、**利用料金は無料**。保存容量・保存期間・利用ユーザー数の制限がなく、マネーフォワード クラウドを契約していない事業者でも利用できる | [クラウドBox 製品ページ](https://biz.moneyforward.com/box/)、[提供開始のプレスリリース](https://corp.moneyforward.com/news/release/service/20211029-mf-press/) |
| 2 | **対応区分は「スキャナ保存」〇・「電子取引データ保存」〇・「電子帳簿等保存」✕。** 帳簿書類では**取引関係書類（請求書・領収書等）〇**・**電子で授受した書類〇**、国税関係帳簿と決算関係書類は✕ | [電子帳簿保存法への対応について](https://biz.moneyforward.com/support/box/guide/e-book/eb01.html) |
| 3 | **タイムスタンプは「アップロード」機能等でファイルの保存時に付与される仕組み** | 同上 |
| 4 | **ヴァージョン管理（訂正又は削除の事実及び内容の確認）は、「スキャナ保存区分に該当する可能性のあるファイルを削除できない仕様」により要件を満たす** | 同上 |
| 5 | **2025年11月28日に API の提供を開始。** 外部システムからの**アップロード・ダウンロード・検索**ができる | [APIの提供を開始しました](https://biz.moneyforward.com/support/box/news/new-feature/20251128.html) |
| 6 | ⚠ **API は「お客さまからのご要望を受け、トライアルとして提供」するものであり、「今後、利用条件等を設ける場合があります」と明記されている** | 同上 |
| 7 | ⚠ **開発者サイトの公開 API 一覧に クラウドBox は載っていない**（掲載は クラウド会計・経費・請求書・債務支払・連結会計と、クラウド会計の MCP サーバー）。**仕様書が公開されていない** | [マネーフォワード クラウド 開発者サイト](https://developers.biz.moneyforward.com/) |
| 8 | 公開 API の認証方式は 2 つ。**OAuth 2.0 による認可**（事業者単位の連携）と、**APIキーを JWT に交換する方式**（発行者の権限で呼ぶ。バックエンドからの実行向け） | 同上 |

**契約書PDFは「電子で授受した書類」に当たるため、#2 の対応範囲に収まる。**

### 確かめること（Phase 0）— **API の1点だけ**

**電帳法を満たすかどうかはもう論点ではない。** 残るのは、**自動アップロードという
選択肢を持てるか**である。

| # | 確認事項 | なぜ効くか |
| --- | --- | --- |
| 1 | **API の仕様書と利用条件を入手する**（開発者サイトに掲載がないため、MF に問い合わせる） | 仕様が読めなければ実装の見積もりが立たない（→ #7） |
| 2 | **トライアル提供の位置づけ。今後どういう利用条件が付きうるか** | **トライアルの API に業務を載せると、条件が付いた時点で止まる**（→ #6）。**これが「依存しない」最大の理由になった** |
| 3 | **無料の利用範囲で API が叩けるか。認証方式（OAuth 2.0／APIキー→JWT）とスコープ** | **クラウド契約で同じ罠を踏んでいる。** 有効なトークンで全エンドポイントが 403 だった（→ §1.3）。**「機能がある」と「自分の契約で叩ける」は別**。アプリポータルの「利用可能サービス」「アプリ連携権限」に**クラウドBox が出るか**をまず見る（→ §1.2 の判定方法） |
| 4 | **実際にトークンを取って1回叩く** | [CODING_STANDARDS.md §10](CODING_STANDARDS.md)。仕様書にあることと、叩けることは別である |

### 設計上の扱い — **依存しない**

**理由が変わった。** 「電帳法を満たすか分からないから」ではなく、
**「API がトライアル提供であり、かつ Drive で成立しているから」**である。

- ツールは**PDFを作って Drive へ保存するだけ**である。**Box へは手で上げても成立する**
  （むしろ、手で上げる運用なら **API の可否と無関係に今日から Box を使える**）
...
> **2026-08-17 改訂: C・D・F を一次ソースに当たって解消し、B を絞り込んだ。**
> 未確定として残るのは **A（顧問弁護士の判断待ち）・B の API 部分・E（公布待ち）**で、
> **いずれも実装の着手を妨げない。**

**本線の未確定事項として A〜F の6つを起票し、うち3つ（C・D・F）は解消済みである。**
それ以外は本線から外れた調査の記録である。
...
| **B** | **クラウドBox の API が使えるか**（仕様書の入手／トライアル提供の位置づけ／無料の範囲で叩けるか） | **選択肢が増えるかどうか。保存要件の充足そのものには関わらない** | MF への問い合わせ・**実際にトークンを取って1回叩く** | **✅ 電帳法の充足は前提として確定（2026-08-17）。** 公式ページで対応区分（スキャナ保存〇・電子取引データ保存〇）とタイムスタンプ付与・削除不可仕様を確認した。**残るのは API の1点。** API は 2025-11-28 提供開始だが**トライアル提供で仕様書が公開されていない**（→ §3） |
| **C** | **業務報酬基準の告示（第8号・第670号）の別表と算定式** | **報酬算定の実装そのもの** | 告示の原文（国土交通省） | **✅ 解消（2026-08-17）。** 告示第8号・第670号・技術的助言の原文を取得し、算定方法・費目の構成・略算方法の倍数（**1.1 / 1.0**）・別表・難易度係数・**略算方法が使えない場合**を確定させた。**告示に定めのない部分は事務所の決めごととして明示的に確定させた**（→ [06-fee-estimation.md §3](06-fee-estimation.md)） |
| **D** | **規則17条の39第1項1号のイ／ロの文言**（メール添付での送信に通知が要らないこと） | 交付の運用 | e-Gov 法令検索の原文 | **✅ 解消（2026-08-17）。** e-Gov 法令API の原文で照合。**イ＝送信して受信者のファイルに記録する方法（メール添付）、ロ＝閲覧に供する方法（ダウンロード）**であり、**通知（2項3号）が課されるのはロだけ**。想定どおり（→ [01-legal-requirements.md §4.2](01-legal-requirements.md)） |
| **E** | **改正法の施行期日政令**と、施行に伴う政省令改正（規則17条の38の記載事項、規則17条の39の技術基準）。**IT重説実施マニュアルの改定**（令和3年9月1日版のまま） | スケジュールと法定必須項目 | 国土交通省の公布を待つ | **未解決（公布待ち）。2026-08-17 時点で、国交省の改正法のページに施行期日政令の掲載はない**（旧 #12・#18） |
| **F** | **建築士法24条の4（帳簿の備付け等及び図書の保存）** — 業務に関する帳簿と設計図書の保存義務。**契約書類の保存とは別の義務** | 保存の運用ルール | 条文と施行規則の原文 | **✅ 解消（2026-08-17）。** 規則21条の原文を確認。**帳簿は各事業年度末に閉鎖し、翌日から15年間保存。図書は作成（受領）から15年間保存。** 帳簿の記載事項8項目のうち6つは契約書ツールの入力そのもの（→ [07-operations.md §4.1](07-operations.md)） |
...
> - **B（クラウドBox）** は**電帳法の充足が前提として置ける**（→ §3）。
>   残る API は**選択肢を増やすだけ**で、Drive で成立する。手で上げる運用なら API は要らない
> - ~~**C（告示の別表）**~~ **解消済み。** 原文から算定式・別表・難易度係数を確定させた
> - ~~**D（イ／ロ）**~~ **解消済み。** メール添付での送信に通知は要らないことを原文で確認した
> - **E（政省令）** は公布待ち。**待っているあいだに作れるものを作る**
> - ~~**F（法24条の4）**~~ **解消済み。** 帳簿・図書とも15年保存。運用ルールへ統合する
